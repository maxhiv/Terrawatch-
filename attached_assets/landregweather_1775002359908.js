  ...
  try {
    const { data } = await axios.get(OPENMETEO_URL, {
      params: {
        latitude:          lat,
        longitude:         lon,
        hourly:            'temperature_2m,relative_humidity_2m,precipitation,wind_speed_10m,wind_direction_10m,surface_pressure,cape,shortwave_radiation,uv_index,lifted_index,soil_moisture_0_to_1cm,convective_inhibition,boundary_layer_height',
        daily:             'temperature_2m_max,temperature_2m_min,precipitation_sum,wind_speed_10m_max,wind_direction_10m_dominant,precipitation_probability_max,uv_index_max,sunshine_duration,shortwave_radiation_sum',
        wind_speed_unit:   'ms',
    ...
    return {
      ...
      current:     data.hourly ? {
        ...
        solar_rad_wm2:data.hourly.shortwave_radiation?.[0],   // W/m² — direct bloom photosynthesis driver
        uv_index:     data.hourly.uv_index?.[0],              // algae growth rate multiplier
        lifted_index: data.hourly.lifted_index?.[0],          // atmospheric instability (negative = storm)
        soil_moisture:data.hourly.soil_moisture_0_to_1cm?.[0],// watershed runoff amplifier
        cin:          data.hourly.convective_inhibition?.[0], // storm suppression
        blh:          data.hourly.boundary_layer_height?.[0], // atmospheric mixing depth
      } : null,
      dailyForecast: data.daily?.time?.map((t, i) => ({
        ...
        precip_sum_mm:data.daily.precipitation_sum?.[i],
        maxWind_ms:   data.daily.wind_speed_10m_max?.[i],
        windDir:      data.daily.wind_direction_10m_dominant?.[i],
        precipProb:   data.daily.precipitation_probability_max?.[i],
        uv_max:       data.daily.uv_index_max?.[i],           // peak UV — bloom growth potential
        sunshine_hrs: data.daily.sunshine_duration?.[i] != null ? data.daily.sunshine_duration[i] / 3600 : null,
        solar_sum_wm2:data.daily.shortwave_radiation_sum?.[i],
      })),
    }
  } catch(err) {
