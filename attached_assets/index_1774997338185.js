...
cron.schedule('*/3 * * * *', async () => {
  try {
    const [usgs, noaa, hfRadar, nerrs, aqi, goesLatest] = await Promise.allSettled([
      getUSGSData(),
      getNOAAData(),
      getCurrentSummary(),
      getWeeksBayLatest(),
    ...
    // Merge fast sources into _latestData, keep last-known slow data intact
    _latestData = {
      ..._latestData,
      waterQuality: { usgs: usgs.value || [], coops: noaa.value || {} },
      hfRadar:      hfRadar.value   ?? null,
      nerrs:        nerrs.value     ?? null,
      aqi:          aqi.value       ?? null,
      goesLatest:   goesLatest.value ?? null,
    }
    ...
    if (result.ok) {
      ...
      console.log(
        ...
        goesTag + ecoTag + satTag + landTag + airTag
      )
    }
  } catch (err) {
...
║  ML vector  → 80+ features from 13 live data sources     ║
