...
export function buildFeatureVector(data = {}) {
  ...
  for (const s of usgs) {
    stationMap[s.siteNo] = {
      ...
      gage:   safeNum(s.readings?.gage_height_ft),       // flood stage proxy
      orthoP: safeNum(s.readings?.orthophosphate_mg_l),  // direct phosphorus loading
      totalN: safeNum(s.readings?.total_nitrogen_mg_l),  // direct nitrogen loading
    }
  }
  ...
  const wb_met = nerrs?.meteorological?.latest  || {}
  const nerrsValues = {
    // ── Water quality station (wekaswq) ──────────────────────────────────────
    wbDo2:    wb.DO_mgl?.value   ?? null,
    wbDOPct:  wb.DO_pct?.value   ?? null,   // % saturation — different from mg/L
    ...
    wbDepth:  wb.Depth?.value    ?? wb.Level?.value ?? null,  // water depth at dock
    // ── Meteorological station (wekmet) — fetched but was silently discarded ─
    wbWSpd:   wb_met.WSpd?.value    ?? null,   // wind speed at bay shore (m/s)
    wbMaxWSpd:wb_met.MaxWSpd?.value ?? null,   // wind gust
    wbWdir:   wb_met.Wdir?.value    ?? null,   // wind direction (°)
    wbATemp:  wb_met.ATemp?.value   ?? null,   // air temperature
    wbBP:     wb_met.BP?.value      ?? null,   // barometric pressure (mb)
    wbPAR:    wb_met.TotPAR?.value  ?? null,   // photosynthetically active radiation — bloom growth driver
    wbPrec:   wb_met.TotPrec?.value ?? null,   // precipitation at station
    wbRH:     wb_met.RH?.value      ?? null,   // relative humidity
  }
  ...
  const tidal = {
    ...
    coops_wind_speed:      safeNum(dauphinIsland.wind?.value),       // knots from CO-OPS wind product
    coops_air_pressure_mb: safeNum(dauphinIsland.air_pressure?.value),
    coops_air_temp_c:      safeNum(dauphinIsland.air_temperature?.value),
  }
  ...
  const buoyFeatures = {
    ...
    buoy_wind_gust_ms:      safeN(buoy?.GST),     // gust — storm transport events
    ...
    buoy_dom_wave_period_s: safeN(buoy?.DPD),     // dominant wave period — mixing energy
    buoy_avg_wave_period_s: safeN(buoy?.APD),
    buoy_mean_wave_dir:     safeN(buoy?.MWD),     // mean wave direction for map layer
  ...
  const weatherFeatures = {
    ...
    nws_wind_gust_mph:  safeN(nwsCurrent.wind_gust_mph),   // gust — bloom transport trigger
  ...
  const landFeatures = {
    ...
    cape_jkg:            safeN(openMeteo?.current?.cape),
    solar_rad_wm2:       safeN(openMeteo?.current?.solar_rad_wm2),  // W/m² — direct bloom photosynthesis driver
    uv_index:            safeN(openMeteo?.current?.uv_index),        // algae growth multiplier
    lifted_index:        safeN(openMeteo?.current?.lifted_index),    // negative = atmospheric instability
    soil_moisture:       safeN(openMeteo?.current?.soil_moisture),   // watershed runoff amplifier
    cin:                 safeN(openMeteo?.current?.cin),             // convective inhibition
    blh:                 safeN(openMeteo?.current?.blh),             // boundary layer height
  ...
  return {
    ...
    // New USGS fields — fetched but previously not in vector
  ...
  try {
    ...
    if (nerrs?.meteorological?.latest) {
      ...
      for (const p of metParams) {
        if (met[p]?.value != null) rows.push([ts, 'nerrs', 'wekmet', p, met[p].value, met[p].unit||''])
      }
    }
